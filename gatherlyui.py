...
class App:
    ...
    def build_main(self):
        ...
        tk.Button(top, text='Create', bg=BTN, font=('Arial', 18),
                  width=10, command=self.show_create).pack(side='left')
    ...
    def render_cards(self):
        ...
        for g in shown:
            ...
            tk.Button(
                card, text=join_txt, state=btn_state,
                command=lambda x=g: self.join_group(x)
            ).grid(row=1, column=2)
    ...
    def build_create(self):
        ...
        tk.Label(self.create, text='Name your study group',
                 bg=BG, font=('Arial', 24)).pack(anchor='w', padx=60, pady=(40, 10))

        self.name_e = tk.Entry(self.create, font=('Arial', 18), width=40)
        self.name_e.pack(padx=60)

        tk.Label(self.create, text='What is the topic?',
                 bg=BG, font=('Arial', 24)).pack(anchor='w', padx=60, pady=(30, 10))

        self.topic_e = tk.Entry(self.create, font=('Arial', 18), width=40)
        self.topic_e.pack(padx=60)

        tk.Label(self.create, text='Date (e.g. June 5th)',
                 bg=BG, font=('Arial', 18)).pack(anchor='w', padx=60, pady=(30, 10))

        self.date_e = tk.Entry(self.create, font=('Arial', 16), width=30)
        self.date_e.pack(padx=60)

        tk.Button(self.create, text='Create Group',
                  font=('Arial', 20), bg=BTN, width=12,
                  command=self.add_group).pack(pady=40)

        tk.Button(self.create, text='Back',
    ...
    def add_group(self):
        ...
        self.joined_ids.add(new_id)
        self.save()

        self.name_e.delete(0, 'end')
        self.topic_e.delete(0, 'end')
    ...
    def open_chat(self, g):
        if g['id'] not in self.joined_ids:
            messagebox.showinfo('Join first',
                                'You need to join this study group first.')
            return
        ...
        tk.Label(win, text=g['name'], bg=BG,
        ...
        tk.Button(entry_frame, text='Send',
                  command=send, bg=BTN).pack(side='left', padx=5)

        if g.get('owner') == 'You':
            admin = tk.Frame(win, bg=BG)
            admin.pack(fill='x', pady=20)

            tk.Label(admin, text='Admin:', bg=BG,
                     font=('Arial', 12, 'bold')).pack(side='left', padx=10)

            tk.Button(admin, text='Delete Group', fg='red',
                      command=lambda: self.delete_group(g, win)).pack(side='left', padx=5)
